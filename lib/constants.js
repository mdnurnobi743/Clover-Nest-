...
// refills it back to this number at midnight BD time).
...
//                (currently min === max on every wedge, i.e. a fixed amount,
//                but the random-range engine is kept so a wedge can be
//                turned into a real range later just by changing min/max)
//   type 'usd' → credits a fixed USD amount, converted to CN via
//                WTC_PER_USD below.
//
// `weight` values are parts-per-100 WINNING CHANCE and MUST sum to exactly
// 100 — that sum is enforced at runtime (see the assertion below) so a
// future edit can't silently break the odds without failing loudly.
//
// Current economy (requested odds):
//   10 CN → 60%   20 CN → 10%   30 CN → 10%   $0.001 → 10%
//   50 CN → 3%    $0.005 → 3%   70 CN → 3%    100 CN → 1%
